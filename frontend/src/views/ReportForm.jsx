...
export default function ReportForm() {
  ...
  const DISEASE_OPTIONS = [
    { id: 'diarrhea', value: 'Diarrhea', label: t('symptom.diarrhea'), defaultSymptoms: ['diarrhea', 'vomiting', 'dehydration', 'abdominal_pain'] },
    { id: 'cholera', value: 'Cholera', label: t('symptom.cholera'), defaultSymptoms: ['diarrhea', 'vomiting', 'dehydration', 'nausea'] },
    { id: 'typhoid', value: 'Typhoid', label: t('symptom.typhoid'), defaultSymptoms: ['fever', 'diarrhea', 'abdominal_pain', 'fatigue', 'loss_of_appetite'] },
    { id: 'hepatitis', value: 'Hepatitis A', label: t('symptom.hepatitis'), defaultSymptoms: ['fever', 'hepatitis', 'nausea', 'fatigue', 'loss_of_appetite'] },
    { id: 'dysentery', value: 'Dysentery', label: 'Dysentery', defaultSymptoms: ['diarrhea', 'fever', 'bloody_stool', 'abdominal_pain'] },
  ...
  useEffect(() => {
    let active = true;
    api.get('/villages')
      .then(({ data }) => { if (active) setVillages(data); })
      .catch(() => { if (active) setVillagesError(t('reportForm.villagesLoadError')); })
      .finally(() => { if (active) setVillagesLoading(false); });
    return () => { active = false; };
  }, [t]);
  ...
  const handleSubmit = async (event) => {
    ...
    const disease = DISEASE_OPTIONS.find((option) => option.id === form.disease_type);
    const diseaseType = form.disease_type === 'other' ? form.disease_type_custom.trim() : disease?.value;
