...
export default function Intelligence() {
  ...
  useEffect(() => {
    if (!user?.district) return undefined;
    let active = true;
    api.get(`/dashboard/predictions/${encodeURIComponent(user.district)}/${encodeURIComponent(disease)}`)
      .then(({ data }) => { if (active) setPrediction(data); })
      .catch((requestError) => {
        if (active) setError(requestError.response?.data?.detail || 'Unable to generate an outbreak forecast right now.');
      })
      .finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [disease, user?.district]);

  const selectDisease = (event) => {
    setLoading(true);
    setError('');
    setActionState('');
    setPrediction(null);
      ...
      <section className="rounded-2xl bg-gradient-to-r from-slate-950 via-blue-950 to-cyan-900 p-6 text-white shadow-lg">
        <div className="flex flex-col justify-between gap-5 md:flex-row md:items-start">
          ...
          <label className="block text-sm font-medium text-slate-100">
            Disease to monitor
            <select value={disease} onChange={selectDisease} className="mt-1 block w-48 rounded-lg border border-slate-500 bg-slate-900 px-3 py-2 text-white">
