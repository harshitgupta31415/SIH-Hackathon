...
const WATER_PARAMS = [
  ...
  { key: 'coliform_count', label: 'Coliform Count', min: 0, max: 1000, step: 1, unit: 'CFU/100ml', ideal: '< 10', icon: '🦠' },
...
export default function WaterQuality() {
  ...
  useEffect(() => {
    api.get('/water-quality')
      ...
      .catch((requestError) => setError(requestError.response?.data?.detail || t('dashboard.loadError')))
  ...
  useEffect(() => {
    api.get('/villages')
      ...
      .catch((requestError) => setError(requestError.response?.data?.detail || t('reportForm.villagesLoadError')));
  }, [t]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!selectedVillage) {
        setError(t('waterQuality.villageError'));
        return;
      }
      const payload = {
        village_id: form.village_id,
        source_type: form.source_type,
        test_date: `${form.test_date}T00:00:00`,
        latitude: selectedVillage.latitude,
        longitude: selectedVillage.longitude,
        notes: form.notes.trim() || null,
      };
      WATER_PARAMS.forEach((p) => {
        if (form[p.key] !== undefined && form[p.key] !== '') {
          payload[p.key] = p.key === 'coliform_count'
            ? Number.parseInt(form[p.key], 10)
            : Number.parseFloat(form[p.key]);
        }
      });
      ...
      setError(err.response?.data?.detail || t('waterQuality.submitFailed'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        {['asha_worker', 'block_officer'].includes(user?.role) && (
