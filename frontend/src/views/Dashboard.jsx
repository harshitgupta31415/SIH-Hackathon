    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/alerts" className="text-sm text-blue-600 hover:underline">{t('dashboard.viewAll')}</Link>
          </div>
      ...
      <div className="card p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {['volunteer', 'asha_worker'].includes(user?.role) && (
            <Link href="/reports/new" className="p-4 bg-blue-50 rounded-lg text-center hover:bg-blue-100 transition-colors">
              <span className="text-2xl">➕</span>
              <p className="text-sm font-medium text-blue-900 mt-2">{t('dashboard.newReport')}</p>
            </Link>
          )}
          <Link href="/risk-map" className="p-4 bg-purple-50 rounded-lg text-center hover:bg-purple-100 transition-colors">
            <span className="text-2xl">🗺️</span>
            <p className="text-sm font-medium text-purple-900 mt-2">{t('dashboard.riskMap')}</p>
          </Link>
          {['asha_worker', 'block_officer', 'district_admin'].includes(user?.role) && (
            <Link href="/water-quality" className="p-4 bg-cyan-50 rounded-lg text-center hover:bg-cyan-100 transition-colors">
              <span className="text-2xl">💧</span>
              <p className="text-sm font-medium text-cyan-900 mt-2">{t('dashboard.waterTest')}</p>
            </Link>
          )}
          <Link href="/alerts" className="p-4 bg-red-50 rounded-lg text-center hover:bg-red-100 transition-colors">
            <span className="text-2xl">🔔</span>
